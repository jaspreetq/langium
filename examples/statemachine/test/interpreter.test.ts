...
describe('Statemachine Interpreter Tests', () => {
    ...
    testCases.forEach(({ description, fileName, events, expectedState, expectedAttributes }) => {
        test(description, async () => {
            // Step 1: Read the file content from the examples directory
            const fileContent = readExampleFile(fileName, examplesDir);

            // Step 2: Create a new file with the specified name and content
            fs.writeFileSync(fileName, fileContent, 'utf8');

            // Step 3: Extract the AST node from the file
            ...
            // Step 4: Interpret the state machine
